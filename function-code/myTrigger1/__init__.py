...
STORAGE_CONN = os.getenv("STORAGE_CONN")
CONTAINER_NAME = os.getenv("STORAGE_CONTAINER_NAME", "incoming")  # Replace default if needed
...
def main(event: func.EventGridEvent):
    ...
    try:
        ...
        # Extract blob name (e.g., xyzphar202502.csv)
        blob_name = blob_url.split("/")[-1]
        company = blob_name[:3]
        dataset_type = blob_name[3:7]  # e.g., phar, medi, demo
        period = blob_name[7:13]

        logging.info(f"Parsed info - Company: {company}, Dataset: {dataset_type}, Period: {period}")

        # Expected relative paths
        expected_files = {
            "phar": f"incoming/dassscrub/pharmacy/{company}/{company}phar{period}.csv",
            "medi": f"incoming/dassscrub/medical/{company}/{company}medi{period}.csv",
            "demo": f"incoming/dassscrub/demo/{company}/{company}demo{period}.csv"
        }
        ...
        for key, blob_path in expected_files.items():
            blob_client = container_client.get_blob_client(blob_path)
            if not blob_client.exists():
                missing.append(blob_path)

        if not missing:
            logging.info(f"✅ All dependencies exist for {company} - {period}. Triggering downstream...")
            # TODO: Trigger downstream (e.g., Service Bus, Airflow)
        else:
            logging.warning(f"⚠️ Missing files for {company} - {period}: {missing}")
